...
def make_totem(center, height, size, column_colorA, column_colorB):
    for i in range(4):
        make_square_circle((center[0], center[1] + (10 + i * 16) / 57 * height, center[2]), size, column_colorB, column_colorA)
        make_circle((center[0] + size + 1, center[1] + (5 + i * 16) / 57 * height, center[2]), 2, column_colorA)
    for m, i, j in ti.ndrange((-size, size+1), (1, height), (-size, size+1)):
        if 9/57*height<=i<16/57*height or 25/57*height<=i<32/57*height or 41/57*height<=i<=48/57*height :        continue;
...
def initialize_voxels():
    ...
    make_totem((50, -50, -28), 57, 4, (0.7, 0.7, 0.1), (0.7, 0.35, 0.1))
initialize_voxels()
scene.finish()
